...
def check_move(grid, x1, y1, x2, y2):
    ...
    #if we are out of bounds (out calling to move over x more than there are columns)
    if x2 > columns - 1 or x2 < 0:
        value = False
    # if we are out of bounds (out calling to move y more than there are rows)
    ...
    # check if the new location has a None
        ...
        value = True
        #because the new location has a None, let's make sure there is not an overhang
        # check if there is an overhang/corner rule to the left
        ...
        # check if there is an overhang/corner rule to the right
...
def do_gravity(grid, x, y):
    ...
    #check to make sure we are moving sand and to make sure we are not at the bottom of
    #the grid
    if grid[y][x] == 's' and y < len(grid)-1 and x < len(grid[0])-1:
    # check if we can go down
        if grid[y + 1][x] == None:
            checked = check_move(grid, x, y, x, y + 1)
            if checked == True:
                return do_move(grid, x, y, x, y + 1)

        # check if we can go down left
        if grid[y + 1][x - 1] == None:
            checked = check_move(grid, x, y, x - 1, y + 1)
            if checked == True:
    ...
    if grid[y][x] == 's' and y < len(grid)-1 and x < len(grid[0])-1:
        ...
        if num < brownian:
            ...
            if coin == 0:
                ...
                if grid[y][x-1] == None:
                    checked = check_move(grid, x, y, x - 1, y)
                    if checked == True:
                        return do_move(grid, x, y, x - 1, y)
            else:
                #move right
                if grid[y][x + 1] == None:
                    checked = check_move(grid, x, y, x + 1, y)
...
def do_whole_grid(grid, brownian):
    ...
    for y in reversed(range(len(grid))):
        for x in reversed(range(len(grid[0]))):
            new_grid = do_gravity(grid,x,y)
            newer_grid = do_brownian(new_grid,x,y,brownian)
    return newer_grid
